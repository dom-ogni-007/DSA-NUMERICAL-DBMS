import javax.swing.*;
import javax.swing.table.DefaultTableModel;
import java.awt.*;
import java.awt.event.ActionEvent;
import java.awt.event.ActionListener;

public class GaussianEliminationGUI {
    private JFrame frame;
    private JTextField numEquationsField;
    private JTextField[][] coefficientFields;
    private JPanel inputPanel;
    private JPanel matrixPanel;
    private JPanel stepPanel;
    private JTextArea solutionArea;
    private int numEquations;

    public static void main(String[] args) {
        EventQueue.invokeLater(() -> {
            try {
                GaussianEliminationGUI window = new GaussianEliminationGUI();
                window.frame.setVisible(true);
            } catch (Exception e) {
                e.printStackTrace();
            }
        });
    }

    public GaussianEliminationGUI() {
        initialize();
    }

    private void initialize() {
        frame = new JFrame("Gaussian Elimination");
        frame.setBounds(100, 100, 800, 600);
        frame.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
        frame.getContentPane().setLayout(new BorderLayout(0, 0));

        inputPanel = new JPanel();
        frame.getContentPane().add(inputPanel, BorderLayout.NORTH);
        inputPanel.setLayout(new GridBagLayout());

        GridBagConstraints gbc = new GridBagConstraints();
        gbc.insets = new Insets(5, 5, 5, 5);
        gbc.gridx = 0;
        gbc.gridy = 0;
        gbc.anchor = GridBagConstraints.WEST;

        JLabel lblNumEquations = new JLabel("Enter the number of equations:");
        inputPanel.add(lblNumEquations, gbc);

        gbc.gridx = 1;
        numEquationsField = new JTextField(10);
        inputPanel.add(numEquationsField, gbc);

        gbc.gridx = 2;
        JButton btnLoadMatrix = new JButton("Load Matrix");
        btnLoadMatrix.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                loadMatrixFields();
            }
        });
        inputPanel.add(btnLoadMatrix, gbc);

        gbc.gridx = 3;
        JButton btnSolve = new JButton("Solve");
        btnSolve.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                solveGaussianElimination();
            }
        });
        inputPanel.add(btnSolve, gbc);

        matrixPanel = new JPanel();
        matrixPanel.setLayout(new BoxLayout(matrixPanel, BoxLayout.Y_AXIS));
        JScrollPane matrixScrollPane = new JScrollPane(matrixPanel);
        frame.getContentPane().add(matrixScrollPane, BorderLayout.CENTER);

        stepPanel = new JPanel();
        stepPanel.setLayout(new BoxLayout(stepPanel, BoxLayout.Y_AXIS));
        JScrollPane stepScrollPane = new JScrollPane(stepPanel);
        frame.getContentPane().add(stepScrollPane, BorderLayout.SOUTH);

        solutionArea = new JTextArea();
        solutionArea.setEditable(false);
        solutionArea.setRows(5);
        frame.getContentPane().add(new JScrollPane(solutionArea), BorderLayout.SOUTH);
    }

    private void loadMatrixFields() {
        try {
            numEquations = Integer.parseInt(numEquationsField.getText());
            matrixPanel.removeAll();

            coefficientFields = new JTextField[numEquations][numEquations + 1];

            for (int i = 0; i < numEquations; i++) {
                JPanel rowPanel = new JPanel(new FlowLayout(FlowLayout.LEFT));
                rowPanel.add(new JLabel("Equation " + (i + 1) + ":"));

                for (int j = 0; j < numEquations + 1; j++) {
                    coefficientFields[i][j] = new JTextField(5);
                    rowPanel.add(coefficientFields[i][j]);
                }
                matrixPanel.add(rowPanel);
            }

            frame.revalidate();
            frame.repaint();
        } catch (NumberFormatException e) {
            JOptionPane.showMessageDialog(frame, "Please enter a valid number of equations.", "Error", JOptionPane.ERROR_MESSAGE);
        }
    }

    private void solveGaussianElimination() {
        double[][] matrix = new double[numEquations][numEquations + 1];

        try {
            for (int i = 0; i < numEquations; i++) {
                for (int j = 0; j < numEquations + 1; j++) {
                    matrix[i][j] = Double.parseDouble(coefficientFields[i][j].getText());
                }
            }
        } catch (NumberFormatException e) {
            JOptionPane.showMessageDialog(frame, "Please enter valid coefficients.", "Error", JOptionPane.ERROR_MESSAGE);
            return;
        }

        stepPanel.removeAll();
        displayMatrix(matrix, "Initial Matrix:");

        double[] solution = gausselim(matrix);

        if (solution != null) {
            solutionArea.setText("Final solution:\n");
            for (int i = 0; i < solution.length; i++) {
                solutionArea.append("x" + (i + 1) + " = " + solution[i] + "\n");
            }
        } else {
            solutionArea.setText("No unique solution exists.");
        }

        frame.revalidate();
        frame.repaint();
    }

   private void displayMatrix(double[][] matrix, String title) {
        DefaultTableModel model = new DefaultTableModel();
        for (int i = 0; i < matrix[0].length; i++) {
            model.addColumn("Col " + (i + 1));
        }
        for (double[] row : matrix) {
            Object[] rowData = new Object[row.length];
            for (int i = 0; i < row.length; i++) {
                rowData[i] = row[i];
            }
            model.addRow(rowData);
        }
        JTable table = new JTable(model);

        JPanel panel = new JPanel(new BorderLayout());
        panel.add(new JLabel(title), BorderLayout.NORTH);
        panel.add(new JScrollPane(table), BorderLayout.CENTER);

        // Add the panel to the matrixPanel
        matrixPanel.add(panel);

        // Revalidate and repaint the matrixPanel
        matrixPanel.revalidate();
        matrixPanel.repaint();
    }

    private double[] gausselim(double[][] matrix) {
        int rows = matrix.length;
        int cols = matrix[0].length;

        for (int k = 0; k < rows - 1; k++) {
            int max = k;
            for (int i = k + 1; i < rows; i++) {
                if (Math.abs(matrix[i][k]) > Math.abs(matrix[max][k])) {
                    max = i;
                }
            }

            double[] temp = matrix[k];
            matrix[k] = matrix[max];
            matrix[max] = temp;

            if (Math.abs(matrix[k][k]) < 1e-10) {
                return null;
            }

            for (int i = k + 1; i < rows; i++) {
                double factor = matrix[i][k] / matrix[k][k];
                for (int j = k; j < cols; j++) {
                    matrix[i][j] -= factor * matrix[k][j];
                }
            }

            displayMatrix(matrix, "After elimination step " + (k + 1) + ":");
        }

        return solve(matrix);
    }

    private double[] solve(double[][] matrix) {
        int rows = matrix.length;
        int cols = matrix[0].length;

        double[] solution = new double[rows];
        for (int i = rows - 1; i >= 0; i--) {
            solution[i] = matrix[i][cols - 1];
            for (int j = i + 1; j < rows; j++) {
                solution[i] -= solution[j] * matrix[i][j];
            }
            solution[i] /= matrix[i][i];
        }
        return solution;
    }
}
    @SuppressWarnings("unchecked")
    // <editor-fold defaultstate="collapsed" desc="Generated Code">//GEN-BEGIN:initComponents
    private void initComponents() {

        setDefaultCloseOperation(javax.swing.WindowConstants.EXIT_ON_CLOSE);

        javax.swing.GroupLayout layout = new javax.swing.GroupLayout(getContentPane());
        getContentPane().setLayout(layout);
        layout.setHorizontalGroup(
            layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
            .addGap(0, 400, Short.MAX_VALUE)
        );
        layout.setVerticalGroup(
            layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
            .addGap(0, 300, Short.MAX_VALUE)
        );

        pack();
    }// </editor-fold>//GEN-END:initComponents

    /**
     * @param args the command line arguments
     */
   

    // Variables declaration - do not modify//GEN-BEGIN:variables
    // End of variables declaration//GEN-END:variables
}
